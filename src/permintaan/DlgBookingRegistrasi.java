        ...
        if(tabMode.getRowCount()==0){
            ...
            Valid.MyReportqry("rptBookingRegistrasi.jasper","report","::[ Laporan Daftar Booking Registrasi ]::",
                "select booking_registrasi.tanggal_booking,booking_registrasi.jam_booking,booking_registrasi.no_rkm_medis, "+
                    ...
                    status+" and dokter.nm_dokter like '%"+TCari.getText().trim()+"%' order by booking_registrasi.no_reg ASC",param);
        }
            ...
            if(akses.getkode().equals("Admin Utama")){
                ...
                if(aktifjadwal.equals("aktif")){
                    if(!kdpnj.getText().trim().equals("BPJ")){
                        ...
                        if(Sequel.cariInteger("select count(no_rkm_medis) from booking_registrasi where kd_dokter='"+KdDokter.getText()+"' and kd_poli='"+KdPoli.getText()+"' and tanggal_periksa='"+Valid.SetTgl(TanggalPeriksa.getSelectedItem()+"")+"' and kd_pj='BPJ' ")>=kuota){
