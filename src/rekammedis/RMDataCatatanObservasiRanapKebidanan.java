...
public final class RMDataCatatanObservasiRanapKebidanan extends javax.swing.JDialog {
    ...
    private String dpjp="", carittv[], caritgl, carijam[];
    ...
    public RMDataCatatanObservasiRanapKebidanan(java.awt.Frame parent, boolean modal) {
        ...
        BJJ.setDocument(new batasInput((byte)15).getKata(BJJ));
                ...
                if(caripemeriksaan.getTable().getSelectedRow()!= -1){
                    try{
                        ps=koneksi.prepareStatement(
                                ...
                                "pemeriksaan_ralan.tgl_perawatan = ? and pemeriksaan_ralan.jam_rawat = ?");
                        try{
                            ps.setString(1,TNoRw.getText().trim());
                            ps.setString(2,caripemeriksaan.getTable().getValueAt(caripemeriksaan.getTable().getSelectedRow(), 0).toString());
                            ps.setString(3,caripemeriksaan.getTable().getValueAt(caripemeriksaan.getTable().getSelectedRow(), 1).toString());
                            rs=ps.executeQuery();
                            ...
                            while(rs.next()){
                                ...
                                Tanggal.setDate(rs.getDate("tgl_perawatan"));
                                carijam = caripemeriksaan.getTable().getValueAt(caripemeriksaan.getTable().getSelectedRow(), 1).toString().split(":");
                                Jam.setSelectedItem(carijam[0].trim());
                                Menit.setSelectedItem(carijam[1].trim());
                                Detik.setSelectedItem(carijam[2].trim());
                            }
                        }catch(Exception ex){
                    ...
                    try{
                        ps=koneksi.prepareStatement(
                                ...
                                "pemeriksaan_ranap.tgl_perawatan = ? and pemeriksaan_ranap.jam_rawat = ?");
                        try{
                            ps.setString(1,TNoRw.getText().trim());
                            ps.setString(2,caripemeriksaan.getTable().getValueAt(caripemeriksaan.getTable().getSelectedRow(), 0).toString());
                            ps.setString(3,caripemeriksaan.getTable().getValueAt(caripemeriksaan.getTable().getSelectedRow(), 1).toString());
    ...
    private void initComponents() {
        ...
        NamaPetugas.setBounds(570, 40, 220, 23);
        ...
        TglLahir.setBounds(689, 10, 130, 23);
        ...
        btnPemeriksaan.setBounds(790, 70, 28, 23);
    ...
    private void btnPemeriksaanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPemeriksaanActionPerformed
        ...
        }else if(ChkKejadian.isSelected()==true){
            JOptionPane.showMessageDialog(null,"Matikan dulu jam otomatis...!!!");
        }else{
