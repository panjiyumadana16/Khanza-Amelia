...
public final class DlgCariCaraBayar2 extends javax.swing.JDialog {
    ...
    */
    ...
    public void tampil3(String pilihKode) {
        Valid.tabelKosong(tabMode);
        try {
            file=new File("./cache/penjab.iyem");
            file.createNewFile();
            ...
            iyem="";
            ps=koneksi.prepareStatement("select * from penjab where penjab.status='1' order by penjab.png_jawab");
            String[] kd_penjab = pilihKode.split(",");
            try{           
                rs=ps.executeQuery();
                while(rs.next()){
                    boolean tercentang = false;
                    for(String kd_pj : kd_penjab){
                        if(kd_pj.equals(rs.getString(1))){
                            tercentang = true;
                            break;
                        }
                    }
                    tabMode.addRow(new Object[]{tercentang,rs.getString(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6)});
                    iyem=iyem+"{\"KodeAsuransi\":\""+rs.getString(1)+"\",\"NamaAsuransi\":\""+rs.getString(2)+"\",\"PerusahaanAsuransi\":\""+rs.getString(3)+"\",\"AlamatAsuransi\":\""+rs.getString(4)+"\",\"NoTelp\":\""+rs.getString(5)+"\",\"Attn\":\""+rs.getString(6)+"\"},";
                }
            }catch(Exception e){
                System.out.println("Notifikasi : "+e);
